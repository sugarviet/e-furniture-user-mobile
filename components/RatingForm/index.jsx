...
const RatingForm = ({ orderBriefInfoCard }) => {
  ...
  return (
    <SafeAreaView className='px-2 h-full'>
      <Text className='text-center font-bold text-2xl my-2'>Leave a Review</Text>
      <CenteredDivider color={COLORS.lightGray} thickness={0.5} />

      <View className='w-full mx-auto flex justify-center px-6'>
        <View key={orderBriefInfoCard.product_id} className="flex flex-row gap-4 pt-2 mb-4">
          <View className="border border-grey4 px-2 py-2 rounded-xl">
            <Image
              resizeMode="contain"
              style={{ width: 90, height: 90 }}
              source={{
                uri: orderBriefInfoCard.thumb
              }}
            />
      ...
      <ScrollView className='mb-2'>
        ...
        <View className='flex items-center gap-1 flex-1 mt-2'>
          ...
          <Text className='text-sm font-bold text-gray-500'>Please give your rating & also share your review</Text>
